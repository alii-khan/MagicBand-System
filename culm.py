...
# Initialize MagicBand System and Parks
disney_system = MagicBandSystem()
magicband1 = MagicBand("MB001")
magicband2 = MagicBand("MB002")
magicband3 = MagicBand("MB003")

disney_system.register_magic_band(magicband1)
disney_system.register_magic_band(magicband2)
disney_system.register_magic_band(magicband3)

# Create sample parks and elements
magic_kingdom = Park("Magic Kingdom")
epcot = Park("Epcot")
animal_kingdom = Park("Animal Kingdom")
hollywood_studios = Park("Hollywood Studios")

ride1 = Ride("Space Mountain", 10)
restaurant1 = Restaurant("Cinderella's Royal Table", ["Salad", "Steak"])
shop1 = Shop("Emporium", ["Mickey Plush", "Magic Band Strap"])
element1 = Element("Castle")

magic_kingdom.add_element(ride1)
magic_kingdom.add_element(restaurant1)
magic_kingdom.add_element(shop1)
magic_kingdom.add_element(element1)

disney_system.add_park(magic_kingdom)
disney_system.add_park(epcot)
disney_system.add_park(animal_kingdom)
disney_system.add_park(hollywood_studios)

# Tkinter Base Class
class DisneyInterface(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Disney Magic Band System")
...
# Employee Interface
class DisneyEmployeeInterface(DisneyInterface):
    def __init__(self):
        super().__init__()
        self.title("Disney Employee Interface")

        tk.Button(self, text="View Park Overview", command=self.view_park_overview).pack()
        tk.Button(self, text="Manage Magic Bands", command=self.manage_magic_bands).pack()

    def view_park_overview(self):
        park_names = [park.name for park in disney_system.parks]
        self.show_message(f"Parks Available: {', '.join(park_names)}")

    def manage_magic_bands(self):
        band_ids = [band.band_id for band in disney_system.magic_bands]
        self.show_message(f"Magic Bands Registered: {', '.join(band_ids)}")

# Guest Interface
class DisneyGuestInterface(DisneyInterface):
    def __init__(self):
        super().__init__()
        self.title("Disney Guest Interface")

        tk.Label(self, text="Enter Magic Band ID:").pack()
        self.band_id_entry = tk.Entry(self)
        self.band_id_entry.pack()
        tk.Button(self, text="View Park Info", command=self.view_park_info).pack()

    def view_park_info(self):
        band_id = self.band_id_entry.get()
        if disney_system.find_magic_band(band_id):
            park_names = [park.name for park in disney_system.parks]
            self.show_message(f"Available Parks: {', '.join(park_names)}")
        else:
            self.show_message("Invalid Magic Band ID")

# Running the Interfaces
def main():
    choice = input("Launch Employee or Guest Interface? (E/G): ").strip().lower()
    if choice == 'e':
        app = DisneyEmployeeInterface()
    else:
        app = DisneyGuestInterface()
    app.mainloop()
